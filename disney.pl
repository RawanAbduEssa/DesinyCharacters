...
menu :-
    write('|*************************************|'),nl,
    write('| Welcome To Our Desiny Expert System |'),nl,
    write('|*************************************|'),nl,
    write('|----------------MENU-----------------|'),nl,
    write('|       1.Guess The Disney Character  |'),nl,
    write('|                                     |'),nl,
    write('|       2.Exit                        |'),nl,
    write('|                                     |'),nl,
    write('|-------------------------------------|'),nl,
    write('|                                     |'),nl,
    write('|choose one of these characters and i |'),nl,
    write('|  will find out which one is on your |'),nl,
    write('|               mind!                 |'),nl,
    write('|                                     |'),nl,
    write('|    Peter Pan, Alice, Tata, Pumbaa   |'),nl,
    write('|                                     |'),nl,
    write('|            Belle, Jane              |'),nl,
    write('|-------------------------------------|'),nl,
...
guess(_) :- write('|                                     |'),nl,
            write('Wrong!! Choose 1 to guess And 2 to Exit'), nl,
            write('|                                     |'),nl.

go() :- hypothesize(Character),
    write('|                                     |'),nl,
    write('I guess that the disney character is: '),nl,
    write('|                                     |'),nl,
    write(Character),nl,
    undo.
...
peterPan :- child, verify(has_no_parents),
           ...
           verify(dont_grow_up),
           verify(friends_with_tinkerbell),
           verify(can_fly),
           verify(wears_green_clothes),
           verify(dealing_with_pirates),
           verify(lives_with_kids),
           verify(lives_in_Neverland).
alice :- child,
